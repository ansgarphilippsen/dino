...
int structObjSet(structObj *obj, Set *set, int flag)
{
  ...
  for(ac=0;ac<obj->atom_count;ac++) {
	...
	case STRUCT_PROP_RAD:
	  if(val->range_flag) {
	    comMessage("\nrange not supported for property radius");
	  } else {
...
int structSmooth2(struct STRUCT_OBJ *obj)
{
  int i;
  struct CGFX_SPLINEC sc;
  double px,py,pz;
  double n1[3],n2[3],n3[3],n4[3];
  float p1[3],p2[3],p3[3],p4[3],p5[3],m[3];
  int bc,cflag,nflag;
  ...
  /*
    the problem is that the object can consist
    of several independent segments, so if 
    necessary, each segment is rendered 
    individually
  */
  ...
  */
  sc.p=Ccalloc(obj->atom_count+2, sizeof(struct CGFX_POINT));

  bc=0;

  /* start display list */
#ifndef USE_VERTEX_ARRAY
  comNewDisplayList(obj->render.list);
#endif
  ...
  while(1) {
    /* 
       step a: collect all points in a segment
       Some render types require calculated points,
       while others use simply positions
       of the CA atoms.
    */
    ...
    if(bc>=obj->bond_count)
      break;
    nflag=0;
    if(obj->render.mode==RENDER_STRAND2) {
      nflag=1;
      /* 
	 first and last point are true Calpha,
	 the rest is interpolated from two
	 consecutive Calpha positions due to
	 the 'wiggling' effect
	 NOTE: the normals must be generated here,
	 since the smoothed out trajectory gives
	 random normals.
      */

      sc.p[0].v[0]=obj->bond[bc].atom1->p->x;
      sc.p[0].v[1]=obj->bond[bc].atom1->p->y;
      sc.p[0].v[2]=obj->bond[bc].atom1->p->z;
      sc.p[0].c[0]=obj->bond[bc].prop1->r;
      sc.p[0].c[1]=obj->bond[bc].prop1->g;
      sc.p[0].c[2]=obj->bond[bc].prop1->b;
      sc.p[0].c[3]=1.0;
      sc.pc=1;
      
      while(bc<obj->bond_count) {
	/* vertice */
	p1[0]=obj->bond[bc].atom1->p->x;
	p1[1]=obj->bond[bc].atom1->p->y;
	p1[2]=obj->bond[bc].atom1->p->z;
	p2[0]=obj->bond[bc].atom2->p->x;
	p2[1]=obj->bond[bc].atom2->p->y;
	p2[2]=obj->bond[bc].atom2->p->z;
	p3[0]=(p1[0]+p2[0])*0.5;
	p3[1]=(p1[1]+p2[1])*0.5;
	p3[2]=(p1[2]+p2[2])*0.5;
	sc.p[sc.pc].v[0]=p3[0];
	...
	/* normal */
	if(sc.pc<2) {
	  /* ignore, will be filled later */
	} else {
	  n1[0]=obj->bond[bc-1].atom1->p->x-obj->bond[bc-1].atom2->p->x;
	  n1[1]=obj->bond[bc-1].atom1->p->y-obj->bond[bc-1].atom2->p->y;
	  n1[2]=obj->bond[bc-1].atom1->p->z-obj->bond[bc-1].atom2->p->z;
	  n2[0]=obj->bond[bc].atom2->p->x-obj->bond[bc].atom1->p->x;
	  n2[1]=obj->bond[bc].atom2->p->y-obj->bond[bc].atom1->p->y;
	  n2[2]=obj->bond[bc].atom2->p->z-obj->bond[bc].atom1->p->z;
	  matCalcCross(n1,n2,n3);
	  if(matCalcDot(n3,n4)<0)
	    matCalcCross(n2,n1,n3);
	  if(matNormalize(n3,n2)!=0) {
	    comMessage("\nnormalize error -2");
	    Cfree(sc.p);
	    return -1;
	  }
	...
	/* color */
	p1[0]=obj->bond[bc].prop1->r;
	p1[1]=obj->bond[bc].prop1->g;
	p1[2]=obj->bond[bc].prop1->b;
	p2[0]=obj->bond[bc].prop2->r;
	p2[1]=obj->bond[bc].prop2->g;
	p2[2]=obj->bond[bc].prop2->b;
	p3[0]=(p1[0]+p2[0])*0.5;
	p3[1]=(p1[1]+p2[1])*0.5;
	p3[2]=(p1[2]+p2[2])*0.5;
	sc.p[sc.pc].c[0]=p3[0];
	...
	if(obj->bond[bc-1].atom2->n!=obj->bond[bc].atom1->n)
      ...
      bc--;
      sc.p[sc.pc].v[0]=obj->bond[bc].atom2->p->x;
      sc.p[sc.pc].v[1]=obj->bond[bc].atom2->p->y;
      sc.p[sc.pc].v[2]=obj->bond[bc].atom2->p->z;
      sc.p[sc.pc].n[0]=n2[0];
      sc.p[sc.pc].n[1]=n2[1];
      sc.p[sc.pc].n[2]=n2[2];
      sc.p[sc.pc].c[0]=obj->bond[bc].prop2->r;
      sc.p[sc.pc].c[1]=obj->bond[bc].prop2->g;
      sc.p[sc.pc].c[2]=obj->bond[bc].prop2->b;
      sc.p[sc.pc].c[3]=1.0;
      sc.pc++;
      bc++;
      
    } else if(obj->render.mode==RENDER_CYLINDER) {
      /* 
	 the cylinder is generated by taken three
	 Calphas in a row, finding the circle-center,
	 and using that value. An beginning and an end
	 point are also formed, extrapolating the first 
	 and last line-segment to the height of the
	 first resp last Calpha
      */
      cflag=0;
      sc.pc=0;
      p1[0]=obj->bond[bc].atom1->p->x;
      ...
      while(bc<obj->bond_count) {
	if(obj->bond[bc-1].atom2->n==obj->bond[bc].atom1->n) {
	  p3[0]=obj->bond[bc].atom2->p->x;
	  p3[1]=obj->bond[bc].atom2->p->y;
	  p3[2]=obj->bond[bc].atom2->p->z;
	  if(matCalcCircleM(p1,p2,p3,m)==0) {
	    if(cflag==2) {
	      sc.p[sc.pc].v[0]=(p4[0]+p5[0]+m[0])/3.0;
	      sc.p[sc.pc].v[1]=(p4[1]+p5[1]+m[1])/3.0;
	      sc.p[sc.pc].v[2]=(p4[2]+p5[2]+m[2])/3.0;
	      sc.p[sc.pc].c[0]=1.0;
	      sc.p[sc.pc].c[1]=1.0;
	      sc.p[sc.pc].c[2]=1.0;
	      sc.p[sc.pc].c[3]=0.5;
      ...
      while(bc<obj->bond_count) {
	if(obj->bond[bc-1].atom2->n==obj->bond[bc].atom1->n) {
	  sc.p[sc.pc].v[0]=obj->bond[bc].atom1->p->x;
	  sc.p[sc.pc].v[1]=obj->bond[bc].atom1->p->y;
	  sc.p[sc.pc].v[2]=obj->bond[bc].atom1->p->z;
	  sc.p[sc.pc].c[0]=obj->bond[bc].prop1->r;
	  sc.p[sc.pc].c[1]=obj->bond[bc].prop1->g;
	  sc.p[sc.pc].c[2]=obj->bond[bc].prop1->b;
	  sc.p[sc.pc].c[3]=1.0;
	  sc.p[sc.pc+1].v[0]=obj->bond[bc].atom2->p->x;
	  sc.p[sc.pc+1].v[1]=obj->bond[bc].atom2->p->y;
	  sc.p[sc.pc+1].v[2]=obj->bond[bc].atom2->p->z;
	  sc.p[sc.pc].c[0]=obj->bond[bc].prop2->r;
	  sc.p[sc.pc].c[1]=obj->bond[bc].prop2->g;
	  sc.p[sc.pc].c[2]=obj->bond[bc].prop2->b;
	  sc.p[sc.pc].c[3]=1.0;
    ...
    */
	...
	if(i==0) {
	  n1[0]=sc.p[0].v[0]-sc.p[1].v[0];
	  n1[1]=sc.p[0].v[1]-sc.p[1].v[1];
	  n1[2]=sc.p[0].v[2]-sc.p[1].v[2];
	  n2[0]=sc.p[2].v[0]-sc.p[1].v[0];
	  n2[1]=sc.p[2].v[1]-sc.p[1].v[1];
	  n2[2]=sc.p[2].v[2]-sc.p[1].v[2];
	} else if(i+1==sc.pc) {
	  n1[0]=sc.p[i-2].v[0]-sc.p[i-1].v[0];
	  n1[1]=sc.p[i-2].v[1]-sc.p[i-1].v[1];
	  n1[2]=sc.p[i-2].v[2]-sc.p[i-1].v[2];
	  n2[0]=sc.p[i+0].v[0]-sc.p[i-1].v[0];
	  n2[1]=sc.p[i+0].v[1]-sc.p[i-1].v[1];
	  n2[2]=sc.p[i+0].v[2]-sc.p[i-1].v[2];
	} else {
	  n1[0]=sc.p[i-1].v[0]-sc.p[i].v[0];
	  n1[1]=sc.p[i-1].v[1]-sc.p[i].v[1];
	  n1[2]=sc.p[i-1].v[2]-sc.p[i].v[2];
	  n2[0]=sc.p[i+1].v[0]-sc.p[i].v[0];
	  n2[1]=sc.p[i+1].v[1]-sc.p[i].v[1];
	  n2[2]=sc.p[i+1].v[2]-sc.p[i].v[2];
	}
	matCalcCross(n1,n2,n3);
...
int structObjTrace(struct DBM_STRUCT_NODE *node, structObj *obj, Select *sel)
{
  ...
  for(pass=0;pass<=1;pass++) {
	...
	if(node->residue[rc].class==STRUCT_PROTEIN) {
	  ...
	  if(!strcmp("C3'",node->residue[rc].atom[ac]->name)){
	    r=structIsAtomSelected(obj->node,node->residue[rc].atom[ac],sel);
	  }
	} else {
	  if(!strcmp("CA",node->residue[rc].atom[ac]->name) ||
