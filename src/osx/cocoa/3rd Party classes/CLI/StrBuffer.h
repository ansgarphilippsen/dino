/* StrBuffer.h Copyright (c) 1998-2001 Philippe Mougin.  */
...
the FIFO sheme: when the store is full and a new string is added, then the "oldest" string in the
store is removed.
...
@interface StrBuffer : NSObject <NSCoding>
{
  NSMutableArray *array;  // an array of NSString
  int head,queue,cursor;
}
...
- initWithUIntSize:(unsigned int)size;  // designated initializer. The capacity of the receiver is
